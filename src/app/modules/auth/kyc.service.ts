...
async function detectAndLabelFaces(imagePath: string, email: string) {
  try {
    ...
    if (
      !(text.toLowerCase().includes('national') ||
      !(text.toLowerCase().includes('republic')) ||
      text.toLowerCase().includes('id') ||
      text.toLowerCase().match(/\b[A-Z0-9]{8}\b/) ) 
    ) {
    ...
    const data = await Kyc.create({ face: des, email });
...
const verifyFace = async (image: string, id: string) => {
  ...
  if (match) {
    await Kyc.findOneAndUpdate({ _id: id }, { status: 'verified' });
  }
